...
This font is half-width ASCII beside full-width ideographs, which is unusual
enough that toolkits disagree about how to classify it. This appendix records what each one actually
...
| Stack | Monospace listing |
|---|---|
| GNOME, GTK 4, Pango | works, `FC_DUAL` accepted |
| MATE, GTK 3, Pango | works, nothing filters |
| KDE Plasma, Qt 6 | needs the scan rule |
| TDE, TQt3, Xft | needs the scan rule |
| Windows GDI | works from the font's own fields |

The scan rule referred to is the one in `65-glowhei.conf`:

```xml
<match target="scan">
  <test name="family"><string>GlowHei Mono</string></test>
