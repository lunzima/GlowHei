...
| 目录 / 文件 | 内容 | 许可 |
|---|---|---|
| `glowhei-1.004/` | 三个字体文件与全部构建源码 | SIL OFL 1.1 |
| `sbitgraft-1.004/` | 去掉 hinting 的工具，单个 C 文件 | MIT |
...
Windows：右键安装 `glowhei-1.004/` 里的 TTC。
...
配置不抢占 `sans-serif` 等泛型族——装一款字体不该替你决定桌面的默认字体，那在
...
文件与技术附录是 CC0，全文见 `LICENSE-CC0`，抄走改用不必署名。`sbitgraft-1.004/`
