...
def gb2312_codepoints() -> frozenset[int]:
    ...
    Microsoft's CP936 maps it to U+00B7. The target is Windows, and the slim
    charset must stay a strict subset of the full one, so both go through
    CP936.
    """
...
#
# It is half width: a single byte is the test this project decides a width by,
# and it is what the source draws too. Sarasa Fixed gives the euro the same 500
# units it gives ASCII, against the Han 1000, so it is half width here before
...
#
# Half width, and the source agrees: Sarasa Fixed draws all 92 at its ASCII 500
# units, against the Han 1000.
...
# walks GB2312's byte ranges and so misses the lot. The slim build is the one
...
# design decision - it puts a half-cell glyph beside the ideographic comma,
# which the same source draws at 1000 - and it is not fixed here, because a
# character list never carries a width. `hansans.py` is where the outlines and
# the advance are both replaced, and `hansans.adopt` is handed this constant.
...
# These twelve the source does draw full width, at the Han 1000. `U+3007` is the
# one that shows up in ordinary text - a date written 二〇二六 needs it.
...
# Box drawing, block elements, and the geometric shapes CP936 carries after
# them. They are in both builds already - `_TABULAR` names the first two and
# the codec reaches the rest - so this constant changes no coverage. It names
...
# where a range cannot.
TABULAR_SYMBOLS = frozenset(range(0x2500, 0x2600))

# The part of that block that *tiles*: box drawing and block elements, the
# characters that have to meet the one beside them and the one on the line
# above. The geometric shapes after them do not tile and are banded with the
# other symbols. This is the same set the sibling project calls CONNECTING.
TILING_SYMBOLS = frozenset(range(0x2500, 0x25A0))


def is_full_width(cp: int) -> bool:
    """True when the codepoint occupies a full cell, one em.

    The test is the CP936 encoded byte count: two bytes means one cell, one
    byte half a cell. It is the rule the sibling project settled on, and for
    the same reason - a list of Unicode blocks leaks, because the CJK
    punctuation that has to line up with the Han is scattered across many of
    them. Enumerating blocks there missed 353 characters, among them the
    ellipsis, the em dash and every accented pinyin vowel.

    Two classes sit outside CP936 and are still full width. CJK Ext A is Han,
    reached only by CP936's successor GB18030; and the box drawing and block
    element ranges are full width throughout, including the parts CP936 omits,
    because a frame that is half a cell wide comes apart at the seam.

    The euro is the mirror case, and returns False: CP936 does carry it, in a
    single byte, and a single byte is half a cell.
    """
    if 0x3400 <= cp <= 0x4DBF or 0x2500 <= cp <= 0x259F:
        return True
    try:
...
def slim() -> frozenset[int]:
    ...
    OS/2 does overstate it. It sets the bit anyway, and `metrics.py` records
    why: a font without it is not merely passed over when an application asks
    for GB2312_CHARSET, it is silently replaced. Tofu on the GBK extensions
    costs less than being unselectable.
...
def full() -> frozenset[int]:
    """Full build, GBK/CP936 level. The default product."""
