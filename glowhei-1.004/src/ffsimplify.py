...
mergelines suit these outlines well: the strokes are largely axis-aligned, so
...
blind to the damage that actually happens: `n`'s shoulder flattens towards a
polygon, the hooks at the foot of a glyph like 慧 slide, and through all of it
the box is unchanged, because the arch still reaches the same height between the
same two stems. Measured against a rendered reference, `n` came out 6.9% away
from its original shape at an error bound of 4.0 while reporting no bulge at
all. So both outlines are rendered unhinted at one pixel to the unit and
compared. The box check is kept as well - a thin spike costs little area but is
worth rejecting.
...
to one part of it: 慧 has fifteen strokes, its foot carries a fifth of the ink,
and a foot the eye reads as bent came to 0.78% over the glyph - under a 1%
check - while the worst eighth-em window in it had changed by 11.75%. The
whole-glyph figure is kept for fits that drift everywhere at once, and it does
still turn down fits the local check passes, but on its own it shipped damage
twice.

A ladder beats simply tightening the bound. The damage is concentrated:
88% of glyphs are within 2% of their shape at the loosest bound, so lowering it
for the whole font pays everywhere for a fault that occurs in a tenth of the
glyphs. Measured over a 1,471-glyph sample, the ladder holds every glyph inside
the tolerance for 4.5% more glyf than the loose bound alone, where a uniform
bound tight enough to match its worst case costs 9% and still has no ceiling.

Some damage does not respond to the bound at all. `n` sits at 1.7% however tight
the fit, because what moves its shoulder is `forcelines` and `choosehv` snapping
near-axis segments onto the axes, and that does not depend on the error bound.
Those glyphs fall through the whole ladder and keep their original outlines,
...
# at once; it is not the one that catches local damage, and on its own it let
# 慧 through with a visibly bent foot.
...
# against sight rather than chosen: over the glyphs examined at the loose
# bound, the ones that looked untouched topped out at 2.0 - 中 0.0, 国 0.4,
# 智 1.2, 全 2.0 - and the ones with damage the eye had already found began at
# 5.1: 及 5.1, 慧 5.5, 球 7.5, 造 11.8, `n` 12.9. Three sits in the gap.
#
# It costs 1.8% of glyf over the whole-glyph check alone, which is the cheapest
# of the corrections this pass has needed.
CELL_TOLERANCE = 3.0
...
# Simple-glyph flag bit 6. Reserved until 2021, OVERLAP_SIMPLE since. Three
# steps here produce it: the source font sets it on 51,584 of its 54,176
# glyphs, FontForge clears almost all of those and leaves about fifteen behind
# on the glyphs it rewrites, and Chlorophytum's writer, ot-builder, sets it
# again on nearly every glyph it emits. Fifteen is already enough for
# `ots-sanitize` 8.2.1 to reject the whole `glyf` table, so it is cleared after
# both steps.
#
